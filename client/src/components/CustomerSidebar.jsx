...
const CustomerSidebar = ({ isOpen = true, onClose }) => {
  ...
  useEffect(() => {
    ...
    container.id = 'customer-sidebar-portal';
  ...
  const menuItems = [
    ...
    { icon: HiOutlineCollection, label: 'Services', path: '/service' },
    { icon: HiOutlineTag, label: 'Categories', path: '/category' },
    ...
    ...(communityFeatures.events
      ...
      : []),
    ...(user?._id
      ? [{ icon: HiOutlineHeart, label: 'Wishlist', path: `/cart/${user._id}` }]
      : []),
    ...(user?._id
      ? [{ icon: HiOutlineUser, label: 'Update Profile', path: `/update-profile/${user._id}` }]
      : []),
    { icon: HiOutlineUserAdd, label: 'Become Provider', path: '/become-provider' },
  ];
  ...
  const sidebarContent = (
    ...
    >
      ...
      >
        <div>
          <h1 className="text-xl font-bold tracking-tight text-gray-900">Commun</h1>
          {user && <p className="mt-1 truncate text-sm text-gray-600">Hello, {getFullName(user)}</p>}
        </div>
        {onClose && (
          <button
            type="button"
